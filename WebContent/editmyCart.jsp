...
<div style="width: auto; height: auto; margin: 0% 8% 10% 5%;">
		...
		<div class="tabPaenl" style="height: 30px;" onclick="orderForm.submit()">Check Out</div>
		<hr style="margin-top: -3px; height: 2px; background-color: #B358C5"/>		

	<form action="placeOrder.jsp" id="orderForm">
	<table width="100%">
...
%>		<!-- color: E8E8E8 -->
		...
		<td><div class="qntDiv"><input type="number" name="<%=rs.getString("Pid") %>" id="<%=rs.getString("Pid") %>" min="1"  max="<%=rs.getString("Quantity") %>" value="<%=request.getParameter(rs.getString("Pid")) %>" onkeyup="calculateEstimateCost('<%=rs.getString("Pid") %>')"  onchange="calculateEstimateCost('<%=rs.getString("Pid") %>')"  onblur="if(<%=rs.getString("Pid") %>.value == '' || isNaN(<%=rs.getString("Pid") %>.value) ) <%=rs.getString("Pid") %>.value = '1'; calculateEstimateCost('<%=rs.getString("Pid") %>')" /></div></td>
		<td><div class="priceDiv"><b>Rs. </b><div style="display: inline-table;" id="<%=rs.getString("Pid") %>Price"><%=rs.getString("Price") %></div> <br/> only /- </div></td>
